...
in vec3 ourColor;
void main(){
vec2 coord = (gl_FragCoord.xy/viewPortSize/mouse_xy.y/100);
float color = 0.0;   

for(float i = 0.0; i < 100*mouse_xy.x; i++){
    
color +=sin(coord.y*50.0 + abs(cos(i+phasor+coord.y*1.0 + cos(coord.y*1.0 + phasor/16.0 * 1.0))))*mouse_xy.y;
...
color= color*sin(i)+cos(i)*mouse_xy.y;
